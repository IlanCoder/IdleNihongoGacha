...
namespace Expeditions.Scriptable {
    ...
    public class BasicExpedition : ScriptableObject {
        ...
        [SerializeField] uint _baseRewards;
        [SerializeField] uint _requiredStats;
        ...
        void CalculateFinalRewards() {
            ...
            float statsScaling = _scalingFactor switch {
                ScalingFactor.Attack => GetPartyAtk() / (float)_requiredStats,
                ScalingFactor.Defence => GetPartyHp() / (float)_requiredStats,
                ScalingFactor.Mixed => GetPartyHp() + GetPartyAtk() / (float)_requiredStats,
